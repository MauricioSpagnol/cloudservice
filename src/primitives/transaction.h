...
static const int8_t  OPOI_RENEW_TX_TYPE             = 11; // F9-E: miner proves continued liveness — resets the stake renewal window and clears canary strikes (opoiMinerAddress/opoiSig/opoiSigTime only, no new fields)
...
// D2 routing-trace dispute (2026-07-26): sentinel for AUDITOR_VERIFY_TX_TYPE's
// (reused) opoiShardIndex field meaning "this verification targets the whole
// RESPONSE", not one EXPERT/DENSE shard's routing-trace commitment. 0 is a
// valid real shardIndex (the same field OPOI_SHARD_RESULT_TX_TYPE already
// carries for that exact purpose), so it can't double as "absent" the way an
// IsNull() hash/COutPoint does for other optional OPoI fields elsewhere.
// Also declared here (rather than opoi.h) for the same two-phase-lookup
// reason OPOI_TASK_VERIFIABLE is: CTransaction::SerializationOp below is a
// template whose body will reference it directly.
static const uint32_t OPOI_AUDITOR_VERIFY_NO_SHARD = std::numeric_limits<uint32_t>::max();
...
public:
    ...
    //
    // D2 routing-trace dispute (2026-07-26): ALSO carried by
    // AUDITOR_VERIFY_TX_TYPE now, optionally — an Auditor replica targeting
    // one specific shard's routing-trace commitment sets this to that
    // shard's real MEG index; a whole-RESPONSE verification (the original,
    // still-supported shape) sets it to the sentinel
    // OPOI_AUDITOR_VERIFY_NO_SHARD instead. Same field, same meaning (index
    // into the request's Model Execution Graph) either way — reused rather
    // than adding a parallel field, since "which shard does this verdict
    // apply to" is exactly what SHARD_RESULT already uses it for.
    const uint32_t             opoiShardIndex;
    // F16: opoiTokenCount reused here too — tokens generated by this shard's
    // (session-wide) compute, feeds the per-token fee component of shard
    // payments the same way it already does for RESPONSE.
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        if (nVersion == FLUXNODE_TX_VERSION) {
            ...
            if (nType == OPOI_REQUEST_TX_TYPE) {
                ...
                // D2 routing-trace dispute (2026-07-26): optional per-shard
                // target — see opoiShardIndex's doc comment above. A
                // whole-response verification (the pre-existing, still fully
                // supported shape) MUST explicitly set this to the sentinel
                // OPOI_AUDITOR_VERIFY_NO_SHARD — CMutableTransaction's own
                // default of 0 for this field would otherwise silently mean
                // "shard 0", which is a real, valid shard index. See
                // submitauditorverification (rpc/opoi.cpp), which sets the
                // sentinel explicitly for its (unchanged) whole-response path.
                ...
                READWRITE(*const_cast<uint32_t*>(&opoiTokenCount));    // F16: tokens generated by this shard
                if (!(s.GetType() & SER_GETHASH))
                    READWRITE(*const_cast<std::vector<unsigned char>*>(&opoiVrfProof));
            } else if (nType == OPOI_RENEW_TX_TYPE) {
                READWRITE(*const_cast<std::string*>(&opoiMinerAddress)); // F9-E: no other fields
            }
            if (!(s.GetType() & SER_GETHASH))
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        if (nVersion == FLUXNODE_TX_VERSION) {
            ...
            if (nType == OPOI_REQUEST_TX_TYPE) {
                ...
                // D2 (2026-07-26): see the const CTransaction template above
                // for the full rationale — same field, same sentinel.
                ...
                READWRITE(opoiResponseHash);
                READWRITE(opoiCommitment);
                READWRITE(opoiTokenCount);   // F16: tokens generated by this shard
                if (!(s.GetType() & SER_GETHASH))
                    READWRITE(opoiVrfProof);
            } else if (nType == OPOI_RENEW_TX_TYPE) {
                READWRITE(opoiMinerAddress); // F9-E: no other fields — no collateral, nothing else to sign over
            }
            if (!(s.GetType() & SER_GETHASH))
                READWRITE(opoiSig);
            return;
