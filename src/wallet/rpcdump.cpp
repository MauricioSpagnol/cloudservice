...
UniValue z_exportwallet(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"folder/filename\"    (string, required) The filename, saved in folder set by csd -exportdir option or specified in input. Exportdir option is primary. If no folder is provided, filename will be saved in folder from which the command is executed.\n"
            "\nResult:\n"
