...
UniValue getmodelgraph(const UniValue& params, bool fHelp)
{
    ...
    for (const auto& d : BuildModelExecutionGraph(m, collapseTitan)) {
        ...
        // D2 (2026-07-26): EXPERT nodes now carry a real (layerStart, layerEnd)
        // scoping them to the dense layer range they fan out from (see
        // BuildModelExecutionGraph's doc comment) — expose both fields for both
        // shard types instead of only for DENSE.
...
UniValue submitshardresult(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 6)
        throw std::runtime_error(
            ...
            "5. router_logits_hash_hex   (string, optional unless this shard_index resolves to an\n"
            "                            EXPERT node in the model's graph (see getmodelgraph) — then\n"
            "                            required: the routing-trace commitment an off-chain Auditor\n"
            "                            checks its own router's choice against (D2, 2026-07-26))\n"
...
UniValue submitauditorverification(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 5 || params.size() > 6)
        throw std::runtime_error(
            ...
            "auto-resolved by ProcessAuditorVerifications exactly like whole-response\n"
            "verdicts are, just tracked independently per (request_id, shard_index) — see\n"
            "mapShardAuditorVerifications' doc comment in opoi.h.\n"
            "\nArguments:\n"
