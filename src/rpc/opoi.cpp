...
UniValue getshardresult(const UniValue& params, bool fHelp)
{
    ...
    // shard. Only computable if request_id's model resolves to an ACTIVE
    // Model Manifest (F15-A/B) — otherwise we have no MEG to classify the
    // shard as DENSE/EXPERT, so fall back to the configured threshold as-is.
    OPoIRequest req;
    if (g_opoiCache.GetRequest(requestId, req)) {
        ModelManifest manifest;
    ...
    bool resolved = g_opoiCache.GetShardMajority(requestId, shardIndex, effectiveMin, majorityHash, divergent);
