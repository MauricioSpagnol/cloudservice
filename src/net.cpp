    ...
    BOOST_FOREACH(CNode* pnode, vNodes)
                {
                    ...
                    if ((Params().NetworkID() == CBaseChainParams::MAIN && pnode->nVersion >= MIN_PEER_PROTO_VERSION) || (Params().NetworkID() == CBaseChainParams::TESTNET && pnode->nVersion >= MIN_PEER_PROTO_VERSION_TESTNET))
                        pnode->PushInventory(inv);
                }
}
