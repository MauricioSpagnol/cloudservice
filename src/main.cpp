                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck, FluxnodeCache* p_fluxnodeCache)
{
    ...
    if (!view.GetSproutAnchorAt(old_sprout_tree_root, sprout_tree))
        return state.DoS(100, error("ConnectBlock(): sprout anchor not found in view — database may be corrupted"),
                         REJECT_INVALID, "bad-sprout-anchor");

    if (sprout_tree.root() != old_sprout_tree_root)
        return state.DoS(100, error("ConnectBlock(): sprout anchor root mismatch"),
                         REJECT_INVALID, "bad-sprout-anchor-root");

    SaplingMerkleTree sapling_tree;
    if (!view.GetSaplingAnchorAt(view.GetBestAnchor(SAPLING), sapling_tree))
        return state.DoS(100, error("ConnectBlock(): sapling anchor not found in view — database may be corrupted"),
                         REJECT_INVALID, "bad-sapling-anchor");
...
bool static DisconnectTip(CValidationState &state, const CChainParams& chainparams, bool fBare = false)
{
    ...
    if (!pcoinsTip->GetSproutAnchorAt(pcoinsTip->GetBestAnchor(SPROUT), newSproutTree))
        return state.DoS(100, error("DisconnectTip(): sprout anchor not found in chainstate — database may be corrupted"),
                         REJECT_INVALID, "bad-sprout-anchor");
    if (!pcoinsTip->GetSaplingAnchorAt(pcoinsTip->GetBestAnchor(SAPLING), newSaplingTree))
        return state.DoS(100, error("DisconnectTip(): sapling anchor not found in chainstate — database may be corrupted"),
                         REJECT_INVALID, "bad-sapling-anchor");
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const CBlock* pblock)
{
    ...
    if (!pcoinsTip->GetSproutAnchorAt(pcoinsTip->GetBestAnchor(SPROUT), oldSproutTree))
        return state.DoS(100, error("ConnectTip(): sprout anchor not found in chainstate — database may be corrupted"),
                         REJECT_INVALID, "bad-sprout-anchor");
    if (!pcoinsTip->GetSaplingAnchorAt(pcoinsTip->GetBestAnchor(SAPLING), oldSaplingTree))
        return state.DoS(100, error("ConnectTip(): sapling anchor not found in chainstate — database may be corrupted"),
                         REJECT_INVALID, "bad-sapling-anchor");
    // Apply the block atomically to the chain state.
...
bool RewindBlockIndex(const CChainParams& chainparams, bool& clearWitnessCaches)
{
    ...
    while (chainActive.Height() >= nHeight) {
        ...
        if (!DisconnectTip(state, chainparams, true)) {
            LogPrintf("RewindBlockIndex: DisconnectTip failed at height %i, stopping rewind\n", pindex->nHeight);
            break;
        }
