...
#define PROCESS_DEP_ENABLE 0x00000001
            ...
            try {
                ...
                if (!fReindex) {
                    ...
                    if (!RewindBlockIndex(chainparams, clearWitnessCaches)) {
                        LogPrintf("WARNING: RewindBlockIndex failed, continuing startup\n");
                    }
                }
    ...
    fFluxnode = GetBoolArg("-csnode", false);
    ...
    if (fFluxnode) {
        ...
        strFluxnodeAddr = GetArg("-csnodeaddr", "");
        ...
        if (!strFluxnodeAddr.empty()) {
            ...
            if (!addrTest.IsValid()) {
                return InitError("Invalid -csnodeaddr address: " + strFluxnodeAddr);
            }
        }

        std::string strHash = GetArg("-csnodeoutpoint", "");
        uint256 hash = uint256S(strHash);
        int index = GetArg("-csnodeindex", -1);
        ...
        strFluxnodePrivKey = GetArg("-csnodeprivkey", "");
         ...
         if (FindBenchmarkPath("csbenchd", strPath)) {
            ...
            if (FindBenchmarkPath("fluxbench-cli",strPath)) {
             ...
             if (FindBenchmarkPath("csbenchd", strPath)) {
                 ...
                 return InitError("Failed to find benchmark application (csbenchd)");
             }
